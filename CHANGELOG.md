...
- **fix: incremental IMAP sync** — startup sync no longer transfers the full UID list from the server when nothing has changed; `sync_all_folders_messages` and `sync_inbox` now use `SEARCH UID {max_uid+1}:*` for folders with existing data so only genuinely new UIDs are fetched; first sync per folder still uses `SEARCH ALL` to bootstrap; `UIDVALIDITY` is now persisted per folder and checked on each sync — if the server resets it the folder is wiped and re-fetched cleanly; `total_exists` from the `SELECT` response replaces the separate `SEARCH ALL` used for message counts. (`core/imap_sync.py`)
