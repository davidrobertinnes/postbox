...
- **feat: reading pane** — email list view is now a Thunderbird/Outlook-style split layout; left column (380px) shows the message list, right pane fills the rest and repopulates on row selection; subject header pinned at pane top, scrollable body below, action buttons (Reply, Forward, Delete, etc.) pinned at bottom; Preview/snippet column removed from list; empty state shown until first selection; j/k keyboard shortcuts now auto-advance rows and update the pane; all email actions (trash, move, spam, star, mark-unread) correctly reset pane state via new `_emCloseView()` helper; partial re-renders (sort, AI triage, mark-all-read) update only the list tbody via `_emRenderListOnly()` without disrupting the pane; det-panel slide-over retained as fallback if pane not in DOM. (`web/static/js/emails.js`, `web/static/postbox.css`, `web/templates/dashboard.html`)
- **feat: resizable list columns** — From, Subject, and Date column headers have drag handles; resizing live-updates column width; widths saved to localStorage and restored on load; `table-layout:fixed` with `text-overflow:ellipsis` on cells. (`web/static/js/emails.js`, `web/static/postbox.css`)

- **perf: STATUS-based sync fast path** — `sync_all_folders_messages` now calls `STATUS (MESSAGES UNSEEN UIDNEXT)` per folder before deciding whether to SELECT; if UIDNEXT hasn't advanced past our stored max UID the folder is skipped entirely (no SELECT, no SEARCH UNSEEN); sidebar counts updated from STATUS directly; SELECT only happens for folders with genuinely new messages. Timing improvement (no new mail): Internode 4.3s→1.3s, Gmail 23s→13s. (`core/imap_sync.py`)
- **fix: incremental IMAP sync** — startup sync no longer transfers the full UID list from the server when nothing has changed; `sync_all_folders_messages` and `sync_inbox` now use `SEARCH UID {max_uid+1}:*` for folders with existing data; results filtered to `u > max_uid` to guard against RFC 3501 range-reversal (when stored max_uid exceeds server's current max, `X:*` inverts and returns old UIDs); `UIDNEXT` from the `SELECT` response used as a fast-path skip when the folder is already caught up; `UIDVALIDITY` persisted and checked — if the server resets it the folder is wiped and re-fetched cleanly; `total_exists` from `SELECT` replaces the separate `SEARCH ALL` used for message counts. (`core/imap_sync.py`)

- **feat: outbox queue** — messages that fail due to network errors are queued and retried automatically rather than lost; `outbox` + `outbox_attachments` tables added to schema; `core/outbox.py` implements queue, retry (max 10 attempts), background loop (60 s interval), and item listing; `/api/send` detects network errors via `is_network_error()` and returns `{queued:true}` instead of an error; new blueprint at `web/routes/outbox.py` (list, count, per-item retry, delete); Outbox page (`web/static/js/outbox.js`) shows status badges (Queued/Failed), error details, per-row Retry/Delete buttons, and "Retry All"; sidebar Outbox nav item with live badge; compose shows "No connection — message queued" toast. (`core/outbox.py`, `core/database.py`, `web/routes/outbox.py`, `web/routes/compose.py`, `web/static/js/outbox.js`, `web/static/js/compose.js`, `web/static/postbox.css`, `web/templates/dashboard.html`, `web/server.py`, `mail.py`)
